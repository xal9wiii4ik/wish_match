...
export function MatchCard({ match, on_open }: MatchCardProps) {
  ...
  const partner_name = partner?.name ?? "Участник";
  ...
  return (
    ...
    >
      <Avatar name={partner_name} src={partner?.avatar_url ?? null} size="md" />

      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="truncate font-semibold text-white">{partner_name}</p>
